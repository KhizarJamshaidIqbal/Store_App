...
@section('content')
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        ...
        <!-- Form -->
        <form action="{{ route('admin.special-offers.update', $specialOffer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="editor mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $specialOffer->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_amount" id="discount_amount"
                           class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                           value="{{ old('discount_amount', $specialOffer->discount_amount) }}">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="start_date" id="start_date"
                               ...
                               value="{{ old('start_date', $specialOffer->start_date) }}" required>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="end_date" id="end_date"
                               ...
                               value="{{ old('end_date', $specialOffer->end_date) }}" required>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 mt-1 cursor-pointer"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="scheduled" {{ old('status', $specialOffer->status) == 'scheduled' ? 'selected' : '' }}>Scheduled</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center mt-6">
                    <input type="checkbox" name="is_featured" id="is_featured"
                           ...
                           {{ old('is_featured', $specialOffer->is_featured) ? 'checked' : '' }}>
            ...
            <div class="mt-6">
                ...
                <textarea name="terms_conditions" id="terms_conditions" rows="4"
                          class="editor mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('terms_conditions', $specialOffer->terms_conditions) }}</textarea>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            flatpickr("#start_date", {
                ...
                defaultDate: "{{ old('start_date', $specialOffer->start_date) }}",
            ...
            flatpickr("#end_date", {
                enableTime: true,
                dateFormat: "Y-m-d H:i",
                defaultDate: "{{ old('end_date', $specialOffer->end_date) }}",
                minDate: "{{ old('start_date', $specialOffer->start_date) }}"
            });
